package com.system.translationpen.hotspot.base;
...
import com.algorithm.progressdialog.ProgressDialog;
...
public abstract class BaseActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    ImageView top_dc_cd;
    @BindView(R.id.top_wifi_hot)
    public ImageView top_wifi_hot;
    @BindView(R.id.top_blue_open)
    ImageView top_blue_open;
     ...
     * 当前应用环境
     */
    public PenApplication mApplication;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mApplication = (PenApplication) getApplication();
        mContext = this;
        //设置layout布局之前
        beforeSetView();
    ...
    protected void onResume() {
        ...
        if (BluetoothController.instance().isBluetoothOpen()){
            top_blue_open.setVisibility(View.VISIBLE);
        }else {
            top_blue_open.setVisibility(View.GONE);
        }
    }
            ...
            if (arg1.getAction().equals(Intent.ACTION_BATTERY_CHANGED)){
                ...
                switch (status) {
                    case BatteryManager.BATTERY_STATUS_CHARGING:
                        /*充电中……*/
                        if (levelPercent<=10){
                            top_dc_cd.setImageResource(R.drawable.top_cdc1);
                        }else if (levelPercent<=20){
                            top_dc_cd.setImageResource(R.drawable.top_cdc2);
                        }else if (levelPercent<=40){
                            top_dc_cd.setImageResource(R.drawable.top_cdc3);
                        }else if (levelPercent<=60){
                            top_dc_cd.setImageResource(R.drawable.top_cdc4);
                        }else if (levelPercent<=80){
                            top_dc_cd.setImageResource(R.drawable.top_cdc5);
                        }else if (levelPercent<=100){
                            top_dc_cd.setImageResource(R.drawable.top_cdc6);
                        }
    ...
    private void phoneSignalStrengthsChanged(){
        final int type = telManager.getNetworkType();
        PhoneStateListener phoneStateListener = new PhoneStateListener() {
            ...
            public void onSignalStrengthsChanged(SignalStrength signalStrength) {
                ...
                try {
                    ...
                    int dbm = (int) method1.invoke(signalStrength);

                    if (isWFW){
                        top_sim_wifi.setVisibility(View.GONE);
                    }else if (dbm==0){
                        top_sim_wifi.setVisibility(View.VISIBLE);
    ...
    private void refreshTime(){
        if (NetworkUtils.getNetworkType()==NetworkUtils.NetworkType.NETWORK_NO){
            ...
            mNetTime=System.currentTimeMillis();
        }else {
